...
from utils import to_real_domain, to_finite_field_domain, finite_field_truncation, ToFiniteFieldDomain, \
    ...
    finite_field_truncation_ext, relu, grad_relu
...
class SimpleNetworkReLU(nn.Module):
    def __init__(self, num_class=10):
        super().__init__()
        self.hidden_layer = nn.Linear(784, 128, bias=False)
        self.output_layer = nn.Linear(128, num_class, bias=False)
        self.relu_act = nn.ReLU()
...
class AbstractNet(ABC):
    def __init__(self, feature_size=784, hidden_layer_size=128, num_classes=10,
                 device=None, verbose=True):
...
class Net(AbstractNet):
    ...
    def _optimizer(self, learning_rate: float):
        self._weight_2 = self._weight_2 - learning_rate * (self.__gradients['weight_2_grad'] / self.__curr_batch_size)
        self._weight_1 = self._weight_1 - learning_rate * (self.__gradients['weight_1_grad'] / self.__curr_batch_size)
    ...
    def _forward_relu(self, input_matrix: torch.Tensor, mode: str = 'train') -> torch.Tensor:
        before_activation = torch.matmul(input_matrix, self._weight_1)
        first_forward = relu(before_activation)
        out = torch.matmul(first_forward, self._weight_2)
        if mode == 'train':
            self.__save_for_backward = {
                'input_matrix': input_matrix,
    ...
    def _backward_relu(self):
        ...
        # weight_1 gradients
        middle_term = -2 * torch.matmul(label - out, torch.t(self._weight_2))
        last_term = middle_term * grad_relu(before_activation)
        weight_1_grad = torch.matmul(torch.t(input_matrix), last_term)
    ...
    def train_mnist(self, data_path: str, num_of_epochs: int, learning_rate: float, batch_size: int):
        self.__batch_size = batch_size
        ...
        target_transform = transforms.Lambda(lambda y: torch.zeros(10, dtype=torch.float)
                                             .scatter_(0, torch.tensor(y), 1))
        # load data
        train_dataset = MNIST(data_path, train=True, transform=transform, target_transform=target_transform,
                              download=True)
        train_loader = DataLoader(train_dataset, batch_size=self.__batch_size, shuffle=True)
    ...
    def train_vgg_cifar10_relu(self, data_path: str, num_of_epochs: int, learning_rate: float, batch_size: int):
        self.__batch_size = batch_size
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
